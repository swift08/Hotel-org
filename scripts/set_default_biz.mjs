...
async function setPrimaryBusiness() {
  ...
  if (u) {
    const { data: curryBiz } = await supabase.from("businesses").select("id").eq("name", "The Curry Courtyard").single();
    ...
    if (curryBiz) {
      ...
      await supabase.from("memberships").update({ is_active: true }).eq("user_id", u.id).eq("business_id", curryBiz.id);

      console.log(`✅ Set 'The Curry Courtyard' (${curryBiz.id}) as primary active membership for ${u.email}!`);
    }
  }
}
