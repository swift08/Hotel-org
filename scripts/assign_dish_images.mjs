...
function getDishImage(item) {
  ...
  if (name.includes("paneer tikka") || name.includes("dahi ke kebab") || name.includes("hara bhara") || name.includes("corn chaat") || name.includes("bhindi")) {
    return ["/images/dishes/paneer_tikka.webp"];
  }
  if (catName.includes("starter") && (name.includes("chicken") || name.includes("seekh") || name.includes("tikka") || name.includes("fish fry") || name.includes("65"))) {
    return ["/images/dishes/chicken_tikka.webp"];
  }
  if (name.includes("tandoori chicken") || name.includes("malai chicken tikka") || name.includes("achari murgh") || name.includes("kebab") || name.includes("grill")) {
    return ["/images/dishes/chicken_tikka.webp"];
  }
  if (name.includes("butter chicken") || name.includes("makhani royale") || name.includes("murgh makhani")) {
    return ["/images/dishes/butter_chicken.webp"];
  }
  if (catName.includes("non-veg") || name.includes("mutton") || name.includes("rogan josh") || name.includes("nihari") || name.includes("curry") || name.includes("rara")) {
    return ["/images/dishes/mutton_curry.webp"];
  }
  if (catName.includes("veg grav") || name.includes("paneer") || name.includes("dal") || name.includes("kofta") || name.includes("chana") || name.includes("aloo gobi")) {
    return ["/images/dishes/paneer_butter_masala.webp"];
  }
  if (catName.includes("south indian") || name.includes("dosa") || name.includes("parotta") || name.includes("kurma")) {
    return ["/images/dishes/masala_dosa.webp"];
  }
  if (catName.includes("bread") || name.includes("naan") || name.includes("roti") || name.includes("paratha") || name.includes("kulcha")) {
    return ["/images/dishes/garlic_naan.webp"];
  }
  if (catName.includes("biryani") || name.includes("biryani") || name.includes("rice") || name.includes("pulao")) {
    if (name.includes("veg") || name.includes("peas")) {
      return ["/images/dishes/veg_biryani.webp"];
    }
    return ["/images/dishes/mutton_biryani.webp"];
  }
  if (catName.includes("beverage") || name.includes("lassi") || name.includes("chaas") || name.includes("lime") || name.includes("mojito") || name.includes("coffee")) {
    return ["/images/dishes/mango_lassi.webp"];
  }
  if (catName.includes("dessert") || name.includes("jamun") || name.includes("rasmalai") || name.includes("halwa") || name.includes("kulfi") || name.includes("cake") || name.includes("brownie")) {
