...
async function fixPermissions() {
  ...
  const rolesToUpdate = ["kitchen_staff", "bar_staff", "chef", "waiter", "floor_manager", "branch_manager", "general_manager"];
  ...
  for (const biz of businesses) {
    ...
    for (const role of rolesToUpdate) {
      for (const perm of permissionsToAdd) {
        ...
        if (error) console.log(`Warning role_permissions (${biz.name}, ${role}, ${perm}):`, error.message);
      }
    }
  }
