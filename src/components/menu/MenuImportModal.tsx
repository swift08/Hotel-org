...
export const MenuImportModal: React.FC<MenuImportModalProps> = ({
  ...
  const handleStartImport = async () => {
    ...
    try {
      ...
      setExcludedText(reviewData.excludedText || []);
        ...
        {step === "review" && (
          <div className="flex-1 flex flex-col lg:flex-row overflow-hidden border-t border-slate-200 dark:border-slate-800">
            ...
            <div className="w-full lg:w-1/2 bg-white dark:bg-slate-900 flex flex-col overflow-hidden">
              ...
              {/* Excluded / Unclassified OCR Debug Panel */}
              {excludedText.length > 0 && (
                <div className="mx-4 mb-3 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden">
                  <button
                    onClick={() => setShowExcluded(!showExcluded)}
                    className="w-full px-4 py-2.5 flex items-center justify-between text-xs font-bold text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-950/60 hover:bg-slate-100 dark:hover:bg-slate-900/60 transition-colors"
                  >
