...
export async function runAuditCore(inputData?: { runSecretScanner?: boolean | undefined }) {
  const results: AuditTestResult[] = [];
    ...
    const { resolveTable, getPublicMenu, getPublicDiningSession, placeOrder, getPublicOrder } = await import("@/lib/public.functions");
    ...
    try {
      ...
      const { data: b1, error: eB1 } = await (supabaseAdmin as any)
        .from("businesses")
        .insert({ name: `Security Test Business 1 (${timestamp})`, slug: `sec-b1-${timestamp}`, currency: "INR", is_active: true })
        .select()
        .single();
      ...
      const { data: b2, error: eB2 } = await (supabaseAdmin as any)
        .from("businesses")
        .insert({ name: `Security Test Business 2 (${timestamp})`, slug: `sec-b2-${timestamp}`, currency: "INR", is_active: true })
        .select()
        .single();
      ...
      const { data: tblA, error: eTblA } = await (supabaseAdmin as any)
        ...
        .insert({ business_id: b1.id, branch_id: br1.id, label: "Table 1", qr_slug: testSlugA, is_active: true, state: "available" })
        .select()
        .single();
      ...
      const { data: tblB, error: eTblB } = await (supabaseAdmin as any)
        .from("restaurant_tables")
        .insert({ business_id: b1.id, branch_id: br1.id, label: "Table 2", qr_slug: testSlugB, is_active: true, state: "available" })
        .select()
        .single();
      ...
      const { data: sessA } = await (supabaseAdmin as any)
        ...
        .insert({ business_id: b1.id, branch_id: br1.id, table_id: tblA.id, session_token: `sess_a_${timestamp}`, status: "active" })
        .select()
        .single();
      ...
      const { data: sessB } = await (supabaseAdmin as any)
        .from("dining_sessions")
        .insert({ business_id: b1.id, branch_id: br1.id, table_id: tblB.id, session_token: `sess_b_${timestamp}`, status: "active" })
        .select()
        .single();
      ...
      const { data: tblT2, error: eTblT2 } = await (supabaseAdmin as any)
        .from("restaurant_tables")
        .insert({ business_id: b2.id, branch_id: br2.id, label: "Table T2", qr_slug: testSlugTenant2, is_active: true, state: "available" })
        .select()
        .single();
      ...
      const { data: prdA, error: ePrdA } = await (supabaseAdmin as any)
        .from("products")
        .insert({ business_id: b1.id, name: "Butter Chicken", base_price: 350, state: "published", is_available: true, station: "kitchen" })
        .select()
        .single();
      ...
      const { data: prdB, error: ePrdB } = await (supabaseAdmin as any)
        .from("products")
        .insert({ business_id: b1.id, name: "Second Special", base_price: 500, state: "published", is_available: true, station: "kitchen" })
        .select()
        .single();
      ...
      const { data: prdT2, error: ePrdT2 } = await (supabaseAdmin as any)
        .from("products")
        .insert({ business_id: b2.id, name: "Tenant 2 Dish", base_price: 200, state: "published", is_available: true, station: "kitchen" })
        .select()
        .single();
    ...
    try {
      ...
      record(1, "Valid QR Slug Resolution", "QR & Resolution", false, "Successful resolution", e.message, "Threw exception", e.message);
    }
    ...
    try {
      ...
      record(2, "Invalid QR Slug Rejection", "QR & Resolution", false, "ok: false", e.message, "Threw exception", e.message);
    }
    ...
    try {
      ...
      record(3, "QR Identifier Unpredictability", "QR & Resolution", false, "Unpredictable slug", e.message, "Error", e.message);
    }
    ...
    try {
      ...
      const pass = verified !== null && verified["businessId"] === createdIds.businessId1 && tamperedVerify === null;
      record(
        4,
      ...
      record(4, "Cookie Cryptographic Signature Integrity", "Session Security", false, "Valid HMAC verification", e.message, "Error", e.message);
    }
    ...
    try {
      ...
      record(5, "BOLA Prevention — Session Mismatch Blocked", "IDOR / BOLA", false, "Mismatch denied", e.message, "Error", e.message);
    }
    ...
    try {
      ...
      record(6, "IDOR Prevention — Cross-Table Order Reading Denied", "IDOR / BOLA", false, "Cross-table order blocked", e.message, "Error", e.message);
    }
    ...
    try {
      ...
      record(7, "Tenant Security — Cross-Tenant Order Access Denied", "Tenant Isolation", false, "Blocked", e.message, "Error", e.message);
    }
    ...
    try {
      ...
      const menuIsolated = ((tenant2Menu as any[])?.length ?? 0) > 0 && createdIds.businessId1 !== createdIds.businessId2;
      record(
        8,
      ...
      record(8, "Tenant Security — Cross-Tenant Menu Isolation", "Tenant Isolation", false, "Menu isolated", e.message, "Error", e.message);
    }
    ...
    try {
      ...
      record(9, "Cross-Tenant Payment Security", "Tenant Isolation", false, "Denied", e.message, "Error", e.message);
    }
    ...
    try {
      ...
      record(10, "Branch Isolation — Cross-Branch Data Segregation", "Branch Isolation", false, "Isolated", e.message, "Error", e.message);
    }
    ...
    try {
      ...
      record(11, "Cross-Branch Session & Table Segregation", "Branch Isolation", false, "Rejected", e.message, "Error", e.message);
    }
    ...
    try {
      ...
      record(12, "Session Lifecycle — Orders Blocked on Completed Session", "Session Lifecycle", false, "Blocked", e.message, "Error", e.message);
    }
    ...
    try {
      ...
      record(13, "Session Lifecycle — Old Session Token After Table Reuse Blocked", "Session Lifecycle", false, "Blocked", e.message, "Error", e.message);
    }
    ...
    try {
      ...
      record(14, "Parameter Tampering — Client Tenant / Branch Payload Ignored", "Parameter Tampering", false, "Ignored", e.message, "Error", e.message);
    }
    ...
    // ──────────────────────────────────────────────────────────
    try {
      const itemToOrder = { productId: createdIds.productIdA!, quantity: 2, clientPrice: 1.00 };
      ...
      record(15, "Price Security — Server-Side Price Recomputation", "Price Security", false, "Price recomputed", e.message, "Error", e.message);
    }
    ...
    try {
      ...
      record(16, "Price Security — Server-Side Tax Computation", "Price Security", false, "Tax secured", e.message, "Error", e.message);
    }
    ...
    try {
      ...
      record(17, "Financial Security — Unauthorized Staff Discount Rejected", "Financial Security", false, "Rejected", e.message, "Error", e.message);
    }
    ...
    try {
      ...
      record(18, "Input Validation — Negative Quantity Rejected", "Input Validation", false, "Rejected", e.message, "Error", e.message);
    }
    ...
    try {
      ...
      record(19, "Input Validation — Extreme Quantity (>50) Rejected", "Input Validation", false, "Rejected", e.message, "Error", e.message);
    }
    ...
    try {
      ...
      record(20, "Financial Security — Refund Exceeding Total Paid Denied", "Financial Security", false, "Denied", e.message, "Error", e.message);
    }
    ...
    try {
      ...
      record(21, "Idempotency — Duplicate Order Submission Prevention", "Idempotency", false, "Deduplicated", e.message, "Error", e.message);
    }
    ...
    try {
      ...
      record(22, "Idempotency — Duplicate Payment Processing Prevention", "Idempotency", false, "Deduplicated", e.message, "Error", e.message);
    }
    ...
    try {
      ...
      record(23, "Staff Security — Anonymous Access Blocked", "Staff Security", false, "Blocked", e.message, "Error", e.message);
    }
    ...
    try {
      ...
      record(24, "Staff Security — Waiter Privilege Escalation Denied", "Staff Security", false, "Escalation denied", e.message, "Error", e.message);
    }
    ...
    try {
      ...
      record(25, "Staff Security — Role Field Tampering Payload Denied", "Staff Security", false, "Role tampering denied", e.message, "Error", e.message);
    }
    ...
    try {
      ...
      record(26, "Staff Security — Disabled Staff Account Blocked", "Staff Security", false, "Blocked", e.message, "Error", e.message);
    }
    ...
    try {
      ...
      record(27, "Audit Trail — Immutable Server Actor Attribution", "Audit Integrity", false, "Audited", e.message, "Error", e.message);
    }
    ...
    try {
      ...
      record(28, "Route Security — /admin Route Protection Check", "Route Security", false, "Protected", e.message, "Error", e.message);
    }
    ...
    if (inputData?.runSecretScanner !== false) {
      try {
        ...
        record(
          ...
          cleanBundle ? "Verified clean! No secrets exposed in client bundle." : leakedSecrets.join("; ")
        );
      } catch (e: any) {
        record(29, "Production Secret Scanner — Client Bundle Audit", "Secret Audit", false, "Clean bundle", e.message, "Error scanning source", e.message);
      }
    }
    ...
    try {
      ...
      const verifiedEntity = admarkDetails.copyright.includes("ADMARK DIGITALS") && admarkDetails.website.includes("admarkdigitals.com");
      record(
        30,
      ...
      record(30, "Legal & Corporate Branding — ADMARK DIGITALS Verification", "Branding & Compliance", false, "Verified", e.message, "Error", e.message);
    }
...
export const runSecurityAuditServer = createServerFn({ method: "POST" })
  .validator((input: unknown) => z.object({ runSecretScanner: z.boolean().optional() }).parse(input))
